"""
AdaLoRA v1 — Adaptive Low-Rank Adaptation.
Key difference from LoRA: dynamically allocates rank budget across layers.
Uses SVD parameterization (W = P·Λ·Q) and prunes singular values during training.
Layers that matter more for the task get higher effective rank.

Same training data, prompts, and pipeline as LoRA v6 / DoRA v2.
"""
...
# ─── MODEL SETUP WITH AdaLoRA ───
def setup_model(total_steps=10000):
    ...
    # ── AdaLoRA CONFIG ──
    # init_r: starting rank for ALL layers (same as LoRA baseline)
    # target_r: average rank AFTER pruning — must be high enough that
    #   no individual layer gets pruned to 0 (caused ValueError before)
    # total_step: total training steps — AdaLoRA uses this to schedule
    #   when to START pruning (tinit) and STOP pruning (tfinal)
    # tfinal: step at which pruning stops — set to 2/3 of training so
    #   the final 1/3 trains with stable ranks
    # deltaT: importance score update every N steps
    adalora_config = AdaLoraConfig(
        init_r=16,
        target_r=14,             # very conservative pruning (12 still caused rank=0)
        lora_alpha=32,
        target_modules=["q_proj","k_proj","v_proj","o_proj","down_proj","up_proj","gate_proj"],
        lora_dropout=0.05,
        bias="none",
        task_type="CAUSAL_LM",
        total_step=total_steps,  # pass in so AdaLoRA can schedule pruning
        tinit=500,               # start pruning after 500 warmup steps
        tfinal=int(total_steps * 0.67),  # stop pruning at 2/3 of training
        deltaT=50,               # update ranks every 50 steps (was 10, too frequent)
        beta1=0.85,
        beta2=0.85,
        orth_reg_weight=0.5,
    )
    model.language_model = get_peft_model(model.language_model, adalora_config)
...
def run_training():
    ...
    WEIGHT_DECAY = 0.05
    WARMUP_RATIO = 0.3
    SAVE_DIR = "./egoorient_adalora_weights_v1"
    ...
    target = max(counts[len(counts)//2], 400)
    ...
    # Estimate total_steps BEFORE model setup (AdaLoRA needs it for pruning schedule)
    # Each sample → 4 clean-label + ~0.5 CoT = ~4.5 examples per balanced sample
    estimated_total_steps = int(len(balanced) * 4.5) * EPOCHS
    print(f"Estimated total steps for AdaLoRA scheduling: {estimated_total_steps}")

    # Now setup model with total_steps
    model, tokenizer, device, num_image_token = setup_model(total_steps=estimated_total_steps)
    ...
    print(f"Warmup steps: {warmup_steps} | Method: AdaLoRA")
    ...
    for epoch in range(EPOCHS):
        ...
        for img_path, is_flipped, prompt, answer in tqdm(epoch_ex):
            try:
                ...
                # ── AdaLoRA-specific: update importance scores & prune ranks ──
                model.language_model.update_and_allocate(global_step)
    ...
    # ── Verify no layer was pruned to rank 0 ──
    rank_pattern = model.language_model.peft_config["default"].rank_pattern
    if rank_pattern:
        min_rank = min(rank_pattern.values())
        print(f"AdaLoRA rank pattern: min={min_rank}, max={max(rank_pattern.values())}")
        if min_rank == 0:
            print("[WARNING] Some layers pruned to rank 0! Eval will fail.")
            print("Layers at rank 0:", [k for k,v in rank_pattern.items() if v == 0])
            print("Consider increasing target_r or reducing pruning.")

    print(f"\nSaving AdaLoRA weights to {SAVE_DIR}...")
